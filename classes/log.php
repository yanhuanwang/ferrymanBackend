...
class log extends DbTable
{
	...
	public function __construct()
	{
		...
		// id
		$this->id = new DbField('log', 'log', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// ip_addr
		$this->ip_addr = new DbField('log', 'log', 'x_ip_addr', 'ip_addr', '`ip_addr`', '`ip_addr`', 200, -1, FALSE, '`ip_addr`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->ip_addr->Nullable = FALSE; // NOT NULL field
		$this->ip_addr->Required = TRUE; // Required field
		$this->ip_addr->Sortable = TRUE; // Allow sort
		$this->fields['ip_addr'] = &$this->ip_addr;

		// mobile_phone
		$this->mobile_phone = new DbField('log', 'log', 'x_mobile_phone', 'mobile_phone', '`mobile_phone`', '`mobile_phone`', 200, -1, FALSE, '`mobile_phone`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->mobile_phone->Sortable = TRUE; // Allow sort
		$this->fields['mobile_phone'] = &$this->mobile_phone;

		// event
		$this->event = new DbField('log', 'log', 'x_event', 'event', '`event`', '`event`', 200, -1, FALSE, '`event`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->event->Nullable = FALSE; // NOT NULL field
		$this->event->Required = TRUE; // Required field
		$this->event->Sortable = TRUE; // Allow sort
		$this->fields['event'] = &$this->event;

		// detail
		$this->detail = new DbField('log', 'log', 'x_detail', 'detail', '`detail`', '`detail`', 200, -1, FALSE, '`detail`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->detail->Sortable = TRUE; // Allow sort
		$this->fields['detail'] = &$this->detail;

		// createdAt
		$this->createdAt = new DbField('log', 'log', 'x_createdAt', 'createdAt', '`createdAt`', CastDateFieldForLike('`createdAt`', 0, "DB"), 135, 0, FALSE, '`createdAt`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// updatedAt
		$this->updatedAt = new DbField('log', 'log', 'x_updatedAt', 'updatedAt', '`updatedAt`', CastDateFieldForLike('`updatedAt`', 0, "DB"), 135, 0, FALSE, '`updatedAt`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "`log`";
	}
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->ip_addr->DbValue = $row['ip_addr'];
		$this->mobile_phone->DbValue = $row['mobile_phone'];
		$this->event->DbValue = $row['event'];
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("logview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("logview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "logadd.php?" . $this->getUrlParm($parm);
		else
			$url = "logadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("logedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("logadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("logdelete.php", $this->getUrlParm());
	}
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->event->setDbValue($rs->fields('event'));
	...
	public function renderEditRow()
	{
		...
		// ip_addr
		$this->ip_addr->EditAttrs["class"] = "form-control";
		$this->ip_addr->EditCustomAttributes = "";
		$this->ip_addr->EditValue = $this->ip_addr->CurrentValue;
		$this->ip_addr->PlaceHolder = RemoveHtml($this->ip_addr->caption());
		...
		// event
		$this->event->EditAttrs["class"] = "form-control";
		$this->event->EditCustomAttributes = "";
		$this->event->EditValue = $this->event->CurrentValue;
