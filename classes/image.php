...
class image extends DbTable
{
	...
	public function __construct()
	{
		...
		// id
		$this->id = new DbField('image', 'image', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "`image`";
	}
	...
	public function update(&$rs, $where = "", $rsold = NULL, $curfilter = TRUE)
	{
		...
		$cascadeUpdate = FALSE;
		$rscascade = array();
		if ($rsold && (isset($rs['id']) && $rsold['id'] <> $rs['id'])) { // Update detail field 'image_id'
			$cascadeUpdate = TRUE;
			$rscascade['image_id'] = $rs['id']; 
		}
		if ($cascadeUpdate) {
			if (!isset($GLOBALS["parcel_info"])) $GLOBALS["parcel_info"] = new parcel_info();
			$rswrk = $GLOBALS["parcel_info"]->loadRs("`image_id` = " . QuotedValue($rsold['id'], DATATYPE_NUMBER, 'DB')); 
			while ($rswrk && !$rswrk->EOF) {
				...
				$success = $GLOBALS["parcel_info"]->Row_Updating($rsdtlold, $rsdtlnew);
				if ($success)
					$success = $GLOBALS["parcel_info"]->update($rscascade, $rskey, $rswrk->fields);
	...
	public function delete(&$rs, $where = "", $curfilter = FALSE)
	{
		...
		if (!isset($GLOBALS["parcel_info"])) $GLOBALS["parcel_info"] = new parcel_info();
		$rscascade = $GLOBALS["parcel_info"]->loadRs("`image_id` = " . QuotedValue($rs['id'], DATATYPE_NUMBER, "DB")); 
		$dtlrows = ($rscascade) ? $rscascade->getRows() : array();

		// Call Row Deleting event
		foreach ($dtlrows as $dtlrow) {
			$success = $GLOBALS["parcel_info"]->Row_Deleting($dtlrow);
			if (!$success) break;
		}
		if ($success) {
			foreach ($dtlrows as $dtlrow) {
				$success = $GLOBALS["parcel_info"]->delete($dtlrow); // Delete
				if ($success === FALSE)
					break;
			}
		}
	...
	public function deleteUploadedFiles($row)
	{
		...
		$oldFiles = EmptyValue($row['path']) ? [] : [$row['path']];
		foreach ($oldFiles as $oldFile) {
			if (file_exists($this->path->oldPhysicalUploadPath() . $oldFile))
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("imageview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("imageview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "imageadd.php?" . $this->getUrlParm($parm);
		else
			$url = "imageadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("imageedit.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("imageedit.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("imageadd.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("imageadd.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("imagedelete.php", $this->getUrlParm());
	}
