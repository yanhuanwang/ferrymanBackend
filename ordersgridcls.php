...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function Page_Main() {
		...
		if ($this->CurrentMode <> "add" && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "user") {
			...
			if (!$this->MasterRecordExists) {
				...
				$this->Page_Terminate("userlist.php"); // Return to master page
			} else {
				$user->LoadListRowValues($rsmaster);
				$user->RowType = EW_ROWTYPE_MASTER; // Master row
				$user->RenderListRow();
				$rsmaster->Close();
			}
		}
		...
		if ($this->CurrentMode <> "add" && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "parcel_info") {
			...
			if (!$this->MasterRecordExists) {
				$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record found
				$this->Page_Terminate("parcel_infolist.php"); // Return to master page
			} else {
				$parcel_info->LoadListRowValues($rsmaster);
				$parcel_info->RowType = EW_ROWTYPE_MASTER; // Master row
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x__userid") && $objForm->HasValue("o__userid") && $this->_userid->CurrentValue <> $this->_userid->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_parcel_id") && $objForm->HasValue("o_parcel_id") && $this->parcel_id->CurrentValue <> $this->parcel_id->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_carrier_id") && $objForm->HasValue("o_carrier_id") && $this->carrier_id->CurrentValue <> $this->carrier_id->OldValue)
			return FALSE;
	...
	function LoadFormValues() {
		...
		if (!$this->parcel_id->FldIsDetailKey) {
			$this->parcel_id->setFormValue($objForm->GetValue("x_parcel_id"));
		}
		$this->parcel_id->setOldValue($objForm->GetValue("o_parcel_id"));
		if (!$this->carrier_id->FldIsDetailKey) {
			$this->carrier_id->setFormValue($objForm->GetValue("x_carrier_id"));
		}
		$this->carrier_id->setOldValue($objForm->GetValue("o_carrier_id"));
	...
	function RenderRow() {
		...
		$this->status->ViewCustomAttributes = "";
			...
			$this->parcel_id->EditValue = ew_HtmlEncode($this->parcel_id->CurrentValue);
			if (strval($this->parcel_id->CurrentValue) <> "") {
				$sFilterWrk = "`id`" . ew_SearchString("=", $this->parcel_id->CurrentValue, EW_DATATYPE_NUMBER, "");
			$sSqlWrk = "SELECT `id`, `id` AS `DispFld`, `name` AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld` FROM `parcel_info`";
			$sWhereWrk = "";
			...
			$this->carrier_id->EditValue = ew_HtmlEncode($this->carrier_id->CurrentValue);
			if (strval($this->carrier_id->CurrentValue) <> "") {
				$sFilterWrk = "`id`" . ew_SearchString("=", $this->carrier_id->CurrentValue, EW_DATATYPE_NUMBER, "");
			$sSqlWrk = "SELECT `id`, `id` AS `DispFld`, `username` AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld` FROM `user`";
			$sWhereWrk = "";
	...
	function ValidateForm() {
		...
		if (!$this->parcel_id->FldIsDetailKey && !is_null($this->parcel_id->FormValue) && $this->parcel_id->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->parcel_id->FldCaption(), $this->parcel_id->ReqErrMsg));
		}
		if (!ew_CheckInteger($this->parcel_id->FormValue)) {
			ew_AddMessage($gsFormError, $this->parcel_id->FldErrMsg());
		}
		if (!$this->carrier_id->FldIsDetailKey && !is_null($this->carrier_id->FormValue) && $this->carrier_id->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->carrier_id->FldCaption(), $this->carrier_id->ReqErrMsg));
		}
		if (!ew_CheckInteger($this->carrier_id->FormValue)) {
			ew_AddMessage($gsFormError, $this->carrier_id->FldErrMsg());
		}
	...
	function SetupMasterParms() {
		...
		if ($sMasterTblVar == "user") {
			...
			if ($GLOBALS["user"]->EventCancelled) $this->EventCancelled = TRUE;
			$this->carrier_id->Visible = FALSE;
			if ($GLOBALS["user"]->EventCancelled) $this->EventCancelled = TRUE;
		}
		if ($sMasterTblVar == "parcel_info") {
			$this->parcel_id->Visible = FALSE;
			if ($GLOBALS["parcel_info"]->EventCancelled) $this->EventCancelled = TRUE;
		}
	...
	function SetupLookupFilters($fld, $pageId = null) {
		...
		case "x__userid":
			$sSqlWrk = "";
			$sSqlWrk = "SELECT `id` AS `LinkFld`, `id` AS `DispFld`, `username` AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld` FROM `user`";
			$sWhereWrk = "{filter}";
		...
		case "x_parcel_id":
			$sSqlWrk = "";
			$sSqlWrk = "SELECT `id` AS `LinkFld`, `id` AS `DispFld`, `name` AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld` FROM `parcel_info`";
			$sWhereWrk = "{filter}";
			$fld->LookupFilters = array();
	...
	function SetupAutoSuggestFilters($fld, $pageId = null) {
		...
		case "x__userid":
			$sSqlWrk = "";
			$sSqlWrk = "SELECT `id`, `id` AS `DispFld`, `username` AS `Disp2Fld` FROM `user`";
			$sWhereWrk = "`id` LIKE '{query_value}%' OR CONCAT(COALESCE(`id`, ''),'" . ew_ValueSeparator(1, $this->_userid) . "',COALESCE(`username`,'')) LIKE '{query_value}%'";
			$fld->LookupFilters = array();
		...
		case "x_parcel_id":
			$sSqlWrk = "";
			$sSqlWrk = "SELECT `id`, `id` AS `DispFld`, `name` AS `Disp2Fld` FROM `parcel_info`";
			$sWhereWrk = "`id` LIKE '{query_value}%' OR CONCAT(COALESCE(`id`, ''),'" . ew_ValueSeparator(1, $this->parcel_id) . "',COALESCE(`name`,'')) LIKE '{query_value}%'";
			$fld->LookupFilters = array();
			$fld->LookupFilters += array("s" => $sSqlWrk, "d" => "");
			$sSqlWrk = "";
			$this->Lookup_Selecting($this->parcel_id, $sWhereWrk); // Call Lookup Selecting
		...
		case "x_carrier_id":
			$sSqlWrk = "";
			$sSqlWrk = "SELECT `id`, `id` AS `DispFld`, `username` AS `Disp2Fld` FROM `user`";
			$sWhereWrk = "`id` LIKE '{query_value}%' OR CONCAT(COALESCE(`id`, ''),'" . ew_ValueSeparator(1, $this->carrier_id) . "',COALESCE(`username`,'')) LIKE '{query_value}%'";
			$fld->LookupFilters = array();
			$fld->LookupFilters += array("s" => $sSqlWrk, "d" => "");
			$sSqlWrk = "";
			$this->Lookup_Selecting($this->carrier_id, $sWhereWrk); // Call Lookup Selecting
