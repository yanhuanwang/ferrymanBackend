<?php include_once "parcel_infoinfo.php" ?>
...
$parcel_info_grid = NULL; // Initialize page object first

class cparcel_info_grid extends cparcel_info {
	...
	// Table name
	var $TableName = 'parcel_info';

	// Page object name
	var $PageObjName = 'parcel_info_grid';

	// Grid form hidden field names
	var $FormName = 'fparcel_infogrid';
	...
	function __construct() {
		...
		if (!isset($GLOBALS["parcel_info"]) || get_class($GLOBALS["parcel_info"]) == "cparcel_info") {
			$GLOBALS["parcel_info"] = &$this;

//			$GLOBALS["MasterTable"] = &$GLOBALS["Table"];
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["parcel_info"];

		}
		$this->AddUrl = "parcel_infoadd.php";
		...
		if (!defined("EW_TABLE_NAME"))
			define("EW_TABLE_NAME", 'parcel_info', TRUE);
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function Page_Main() {
		...
		// Load master record
		if ($this->CurrentMode <> "add" && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "image") {
			global $image;
			$rsmaster = $image->LoadRs($this->DbMasterFilter);
			$this->MasterRecordExists = ($rsmaster && !$rsmaster->EOF);
			if (!$this->MasterRecordExists) {
				$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record found
				$this->Page_Terminate("imagelist.php"); // Return to master page
			} else {
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_from_place") && $objForm->HasValue("o_from_place") && $this->from_place->CurrentValue <> $this->from_place->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_to_place") && $objForm->HasValue("o_to_place") && $this->to_place->CurrentValue <> $this->to_place->OldValue)
			return FALSE;
		...
		if ($objForm->HasValue("x_category") && $objForm->HasValue("o_category") && $this->category->CurrentValue <> $this->category->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_image_id") && $objForm->HasValue("o_image_id") && $this->image_id->CurrentValue <> $this->image_id->OldValue)
			return FALSE;
	...
	function LoadFormValues() {
		...
		if (!$this->from_place->FldIsDetailKey) {
			$this->from_place->setFormValue($objForm->GetValue("x_from_place"));
		}
		$this->from_place->setOldValue($objForm->GetValue("o_from_place"));
		if (!$this->to_place->FldIsDetailKey) {
			$this->to_place->setFormValue($objForm->GetValue("x_to_place"));
		}
		$this->to_place->setOldValue($objForm->GetValue("o_to_place"));
		...
		$this->image_id->setOldValue($objForm->GetValue("o_image_id"));
	...
	function RenderRow() {
		...
		$this->name->ViewCustomAttributes = "";
			...
			$this->from_place->EditValue = ew_HtmlEncode($this->from_place->CurrentValue);
			...
			$this->to_place->EditValue = ew_HtmlEncode($this->to_place->CurrentValue);
			...
			$sSqlWrk = "SELECT `id`, `id` AS `DispFld`, `name` AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld`, '' AS `SelectFilterFld`, '' AS `SelectFilterFld2`, '' AS `SelectFilterFld3`, '' AS `SelectFilterFld4` FROM `category`";
			$sWhereWrk = "";
			$this->category->LookupFilters = array();
			ew_AddFilter($sWhereWrk, $sFilterWrk);
			$this->Lookup_Selecting($this->category, $sWhereWrk); // Call Lookup Selecting
			...
			$this->image_id->EditValue = ew_HtmlEncode($this->image_id->CurrentValue);
			if (strval($this->image_id->CurrentValue) <> "") {
				$sFilterWrk = "`id`" . ew_SearchString("=", $this->image_id->CurrentValue, EW_DATATYPE_NUMBER, "");
			$sSqlWrk = "SELECT `id`, `path` AS `DispFld`, '' AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld` FROM `image`";
			$sWhereWrk = "";
			...
			$this->image_id->LinkCustomAttributes = "";
			if (!ew_Empty($this->image_id->CurrentValue)) {
				$this->image_id->HrefValue = "uploads/" . ((!empty($this->image_id->EditValue) && !is_array($this->image_id->EditValue)) ? ew_RemoveHtml($this->image_id->EditValue) : $this->image_id->CurrentValue); // Add prefix/suffix
				$this->image_id->LinkAttrs["target"] = ""; // Add target
				if ($this->Export <> "") $this->image_id->HrefValue = ew_FullUrl($this->image_id->HrefValue, "href");
			} else {
				$this->image_id->HrefValue = "";
	...
	function ValidateForm() {
		...
		if (!EW_SERVER_VALIDATE)
			return ($gsFormError == "");
		if (!$this->from_place->FldIsDetailKey && !is_null($this->from_place->FormValue) && $this->from_place->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->from_place->FldCaption(), $this->from_place->ReqErrMsg));
		}
		if (!$this->to_place->FldIsDetailKey && !is_null($this->to_place->FormValue) && $this->to_place->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->to_place->FldCaption(), $this->to_place->ReqErrMsg));
		}
		...
		if (!$this->image_id->FldIsDetailKey && !is_null($this->image_id->FormValue) && $this->image_id->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->image_id->FldCaption(), $this->image_id->ReqErrMsg));
		}
		if (!ew_CheckInteger($this->image_id->FormValue)) {
			ew_AddMessage($gsFormError, $this->image_id->FldErrMsg());
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$sMasterFilter = $this->SqlMasterFilter_image();
			...
			if ($bValidMasterRecord) {
				if (!isset($GLOBALS["image"])) $GLOBALS["image"] = new cimage();
				$rsmaster = $GLOBALS["image"]->LoadRs($sMasterFilter);
				$bValidMasterRecord = ($rsmaster && !$rsmaster->EOF);
				$rsmaster->Close();
			}
			if (!$bValidMasterRecord) {
				$sRelatedRecordMsg = str_replace("%t", "image", $Language->Phrase("RelatedRecordRequired"));
	...
	function AddRow($rsold = NULL) {
		...
		$sMasterFilter = $this->SqlMasterFilter_image();
		if (strval($this->image_id->CurrentValue) <> "") {
			$sMasterFilter = str_replace("@id@", ew_AdjustSql($this->image_id->CurrentValue, "DB"), $sMasterFilter);
		} else {
			$bValidMasterRecord = FALSE;
		}
		if ($bValidMasterRecord) {
	...
	function SetupMasterParms() {
		...
		if ($sMasterTblVar == "image") {
			$this->image_id->Visible = FALSE;
			if ($GLOBALS["image"]->EventCancelled) $this->EventCancelled = TRUE;
		}
	...
	function SetupLookupFilters($fld, $pageId = null) {
		...
		case "x_category":
			$sSqlWrk = "";
			$sSqlWrk = "SELECT `id` AS `LinkFld`, `id` AS `DispFld`, `name` AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld` FROM `category`";
			$sWhereWrk = "";
			$fld->LookupFilters = array();
		...
		case "x_image_id":
			$sSqlWrk = "";
			$sSqlWrk = "SELECT `id` AS `LinkFld`, `path` AS `DispFld`, '' AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld` FROM `image`";
			$sWhereWrk = "{filter}";
			$fld->LookupFilters = array();
			$fld->LookupFilters += array("s" => $sSqlWrk, "d" => "", "f0" => '`id` IN ({filter_value})', "t0" => "3", "fn0" => "");
			$sSqlWrk = "";
	...
	function SetupAutoSuggestFilters($fld, $pageId = null) {
		...
		case "x_user_id":
			...
			$sWhereWrk = "`id` LIKE '{query_value}%' OR CONCAT(COALESCE(`id`, ''),'" . ew_ValueSeparator(1, $this->user_id) . "',COALESCE(`username`,'')) LIKE '{query_value}%'";
			$fld->LookupFilters = array();
		...
		case "x_image_id":
			$sSqlWrk = "";
			$sSqlWrk = "SELECT `id`, `path` AS `DispFld` FROM `image`";
			$sWhereWrk = "`path` LIKE '{query_value}%'";
			$fld->LookupFilters = array();
			$fld->LookupFilters += array("s" => $sSqlWrk, "d" => "");
			$sSqlWrk = "";
			$this->Lookup_Selecting($this->image_id, $sWhereWrk); // Call Lookup Selecting
