...
$user_grid = NULL; // Initialize page object first

class cuser_grid extends cuser {
	...
	// Page object name
	var $PageObjName = 'user_grid';

	// Grid form hidden field names
	var $FormName = 'fusergrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["user"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_address") && $objForm->HasValue("o_address") && $this->address->CurrentValue <> $this->address->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_country") && $objForm->HasValue("o_country") && $this->country->CurrentValue <> $this->country->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_photo") && $objForm->HasValue("o_photo") && $this->photo->CurrentValue <> $this->photo->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_nickname") && $objForm->HasValue("o_nickname") && $this->nickname->CurrentValue <> $this->nickname->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_region") && $objForm->HasValue("o_region") && $this->region->CurrentValue <> $this->region->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_locked") && $objForm->HasValue("o_locked") && $this->locked->CurrentValue <> $this->locked->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_send_role") && $objForm->HasValue("o_send_role") && $this->send_role->CurrentValue <> $this->send_role->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_carrier_role") && $objForm->HasValue("o_carrier_role") && $this->carrier_role->CurrentValue <> $this->carrier_role->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function LoadFormValues() {
		...
		$this->address->setOldValue($objForm->GetValue("o_address"));
		if (!$this->country->FldIsDetailKey) {
			$this->country->setFormValue($objForm->GetValue("x_country"));
		}
		$this->country->setOldValue($objForm->GetValue("o_country"));
		if (!$this->photo->FldIsDetailKey) {
			$this->photo->setFormValue($objForm->GetValue("x_photo"));
		}
		$this->photo->setOldValue($objForm->GetValue("o_photo"));
		if (!$this->nickname->FldIsDetailKey) {
			$this->nickname->setFormValue($objForm->GetValue("x_nickname"));
		}
		$this->nickname->setOldValue($objForm->GetValue("o_nickname"));
		if (!$this->region->FldIsDetailKey) {
			$this->region->setFormValue($objForm->GetValue("x_region"));
		}
		$this->region->setOldValue($objForm->GetValue("o_region"));
		if (!$this->locked->FldIsDetailKey) {
			$this->locked->setFormValue($objForm->GetValue("x_locked"));
		}
		$this->locked->setOldValue($objForm->GetValue("o_locked"));
		if (!$this->send_role->FldIsDetailKey) {
			$this->send_role->setFormValue($objForm->GetValue("x_send_role"));
		}
		$this->send_role->setOldValue($objForm->GetValue("o_send_role"));
		if (!$this->carrier_role->FldIsDetailKey) {
			$this->carrier_role->setFormValue($objForm->GetValue("x_carrier_role"));
		}
		$this->carrier_role->setOldValue($objForm->GetValue("o_carrier_role"));
	}
	...
	function RenderRow() {
		...
		// send_role
		$this->send_role->ViewValue = $this->send_role->CurrentValue;
		$this->send_role->ViewCustomAttributes = "";

		// carrier_role
		$this->carrier_role->ViewValue = $this->carrier_role->CurrentValue;
		$this->carrier_role->ViewCustomAttributes = "";
			...
			$this->nickname->PlaceHolder = ew_RemoveHtml($this->nickname->FldCaption());
			...
			$this->locked->PlaceHolder = ew_RemoveHtml($this->locked->FldCaption());

			// send_role
			$this->send_role->EditAttrs["class"] = "form-control";
			$this->send_role->EditCustomAttributes = "";
			$this->send_role->EditValue = ew_HtmlEncode($this->send_role->CurrentValue);
			$this->send_role->PlaceHolder = ew_RemoveHtml($this->send_role->FldCaption());

			// carrier_role
			$this->carrier_role->EditAttrs["class"] = "form-control";
			$this->carrier_role->EditCustomAttributes = "";
			$this->carrier_role->EditValue = ew_HtmlEncode($this->carrier_role->CurrentValue);
			$this->carrier_role->PlaceHolder = ew_RemoveHtml($this->carrier_role->FldCaption());
	...
	function ValidateForm() {
		...
		if (!$this->nickname->FldIsDetailKey && !is_null($this->nickname->FormValue) && $this->nickname->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->nickname->FldCaption(), $this->nickname->ReqErrMsg));
		}
		...
		if (!$this->locked->FldIsDetailKey && !is_null($this->locked->FormValue) && $this->locked->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->locked->FldCaption(), $this->locked->ReqErrMsg));
		}
		if (!ew_CheckInteger($this->locked->FormValue)) {
			ew_AddMessage($gsFormError, $this->locked->FldErrMsg());
		}
		if (!$this->send_role->FldIsDetailKey && !is_null($this->send_role->FormValue) && $this->send_role->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->send_role->FldCaption(), $this->send_role->ReqErrMsg));
		}
		if (!ew_CheckInteger($this->send_role->FormValue)) {
			ew_AddMessage($gsFormError, $this->send_role->FldErrMsg());
		}
		if (!$this->carrier_role->FldIsDetailKey && !is_null($this->carrier_role->FormValue) && $this->carrier_role->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->carrier_role->FldCaption(), $this->carrier_role->ReqErrMsg));
		}
		if (!ew_CheckInteger($this->carrier_role->FormValue)) {
			ew_AddMessage($gsFormError, $this->carrier_role->FldErrMsg());
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// nickname
			$this->nickname->SetDbValueDef($rsnew, $this->nickname->CurrentValue, "", $this->nickname->ReadOnly);
