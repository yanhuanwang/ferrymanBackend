...
//
// Table class for trip_info
//
class ctrip_info extends cTable {
	var $id;
	var $from_place;
	var $to_place;
	var $description;
	...
	function __construct() {
		...
		$this->TableVar = 'trip_info';
		$this->TableName = 'trip_info';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`trip_info`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new cField('trip_info', 'trip_info', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->Sortable = TRUE; // Allow sort
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// from_place
		$this->from_place = new cField('trip_info', 'trip_info', 'x_from_place', 'from_place', '`from_place`', '`from_place`', 200, -1, FALSE, '`from_place`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->from_place->Sortable = TRUE; // Allow sort
		$this->fields['from_place'] = &$this->from_place;

		// to_place
		$this->to_place = new cField('trip_info', 'trip_info', 'x_to_place', 'to_place', '`to_place`', '`to_place`', 200, -1, FALSE, '`to_place`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->to_place->Sortable = TRUE; // Allow sort
		$this->fields['to_place'] = &$this->to_place;

		// description
		$this->description = new cField('trip_info', 'trip_info', 'x_description', 'description', '`description`', '`description`', 200, -1, FALSE, '`description`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->description->Sortable = TRUE; // Allow sort
		$this->fields['description'] = &$this->description;

		// user_id
		$this->user_id = new cField('trip_info', 'trip_info', 'x_user_id', 'user_id', '`user_id`', '`user_id`', 3, -1, FALSE, '`user_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->user_id->Sortable = TRUE; // Allow sort
		$this->user_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['user_id'] = &$this->user_id;

		// flight_number
		$this->flight_number = new cField('trip_info', 'trip_info', 'x_flight_number', 'flight_number', '`flight_number`', '`flight_number`', 200, -1, FALSE, '`flight_number`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->flight_number->Sortable = TRUE; // Allow sort
		$this->fields['flight_number'] = &$this->flight_number;

		// date
		$this->date = new cField('trip_info', 'trip_info', 'x_date', 'date', '`date`', ew_CastDateFieldForLike('`date`', 0, "DB"), 133, 0, FALSE, '`date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`trip_info`";
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "trip_infoedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "trip_infoadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("trip_infoview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("trip_infoview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "trip_infoadd.php?" . $this->UrlParm($parm);
		else
			$url = "trip_infoadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("trip_infoedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("trip_infoadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("trip_infodelete.php", $this->UrlParm());
	}
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->flight_number->Exportable) $Doc->ExportCaption($this->flight_number);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->flight_number->Exportable) $Doc->ExportField($this->flight_number);
