...
$image_grid = NULL; // Initialize page object first

class cimage_grid extends cimage {
	...
	// Page object name
	var $PageObjName = 'image_grid';

	// Grid form hidden field names
	var $FormName = 'fimagegrid';
	...
	function __construct() {
		...
		// Table object (image)
		if (!isset($GLOBALS["image"]) || get_class($GLOBALS["image"]) == "cimage") {
			$GLOBALS["image"] = &$this;

//			$GLOBALS["MasterTable"] = &$GLOBALS["Table"];
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["image"];
		...
		if (@$_POST["ajax"] == "autofill") {
			...
			if ($sDetailTblVar <> "") {
				...
				if (in_array("parcel_info", $DetailTblVar)) {
					...
					// Process auto fill for detail table 'parcel_info'
					if (preg_match('/^fparcel_info(grid|add|addopt|edit|update|search)$/', @$_POST["form"])) {
						if (!isset($GLOBALS["parcel_info_grid"])) $GLOBALS["parcel_info_grid"] = new cparcel_info_grid;
						$GLOBALS["parcel_info_grid"]->Page_Init();
						$this->Page_Terminate();
						exit();
					}
				}
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function RenderListOptions() {
		...
		if ($Security->CanAdd()) {
			if (ew_IsMobile())
				...
				$oListOpt->Body = "<a class=\"ewRowLink ewCopy\" title=\"" . $copycaption . "\" data-table=\"image\" data-caption=\"" . $copycaption . "\" href=\"javascript:void(0);\" onclick=\"ew_ModalDialogShow({lnk:this,btn:'AddBtn',url:'" . ew_HtmlEncode($this->CopyUrl) . "'});\">" . $Language->Phrase("CopyLink") . "</a>";
		} else {
			$oListOpt->Body = "";
		}
		} // End View mode
	...
	function SetupOtherOptions() {
		...
		if ($this->CurrentMode == "view") { // Check view mode
			...
			if (ew_IsMobile())
				...
				$item->Body = "<a class=\"ewAddEdit ewAdd\" title=\"" . $addcaption . "\" data-table=\"image\" data-caption=\"" . $addcaption . "\" href=\"javascript:void(0);\" onclick=\"ew_ModalDialogShow({lnk:this,btn:'AddBtn',url:'" . ew_HtmlEncode($this->AddUrl) . "'});\">" . $Language->Phrase("AddLink") . "</a>";
	...
	function LoadDefaultValues() {
		...
		$this->path->Upload->Index = $this->RowIndex;
	...
	function RenderRow() {
		...
		if (!ew_Empty($this->path->Upload->DbValue)) {
			$this->path->ImageAlt = $this->path->FldAlt();
		...
		$this->description->ViewCustomAttributes = "";
			...
			if (!ew_Empty($this->path->Upload->DbValue)) {
				$this->path->HrefValue = ew_GetFileUploadUrl($this->path, $this->path->Upload->DbValue); // Add prefix/suffix
				$this->path->LinkAttrs["target"] = ""; // Add target
				if ($this->Export <> "") $this->path->HrefValue = ew_FullUrl($this->path->HrefValue, "href");
			} else {
				$this->path->HrefValue = "";
			}
			...
			if ($this->path->UseColorbox) {
				if (ew_Empty($this->path->TooltipValue))
					$this->path->LinkAttrs["title"] = $Language->Phrase("ViewImageGallery");
				$this->path->LinkAttrs["data-rel"] = "image_x" . $this->RowCnt . "_path";
				ew_AppendClass($this->path->LinkAttrs["class"], "ewLightbox");
			}
			...
			if (is_numeric($this->RowIndex) && !$this->EventCancelled) ew_RenderUploadField($this->path, $this->RowIndex);
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			if ($this->path->Visible && !$this->path->Upload->KeepFile) {
				$OldFiles = ew_Empty($this->path->Upload->DbValue) ? array() : array($this->path->Upload->DbValue);
				if (!ew_Empty($this->path->Upload->FileName)) {
					$NewFiles = array($this->path->Upload->FileName);
					$NewFileCount = count($NewFiles);
					for ($i = 0; $i < $NewFileCount; $i++) {
						$fldvar = ($this->path->Upload->Index < 0) ? $this->path->FldVar : substr($this->path->FldVar, 0, 1) . $this->path->Upload->Index . substr($this->path->FldVar, 1);
						if ($NewFiles[$i] <> "") {
							$file = $NewFiles[$i];
							if (file_exists(ew_UploadTempPath($fldvar, $this->path->TblVar) . $file)) {
								$file1 = ew_UploadFileNameEx($this->path->PhysicalUploadPath(), $file); // Get new file name
								if ($file1 <> $file) { // Rename temp file
									while (file_exists(ew_UploadTempPath($fldvar, $this->path->TblVar) . $file1) || file_exists($this->path->PhysicalUploadPath() . $file1)) // Make sure no file name clash
										$file1 = ew_UniqueFilename($this->path->PhysicalUploadPath(), $file1, TRUE); // Use indexed name
									rename(ew_UploadTempPath($fldvar, $this->path->TblVar) . $file, ew_UploadTempPath($fldvar, $this->path->TblVar) . $file1);
									$NewFiles[$i] = $file1;
								}
							}
						}
					}
					$this->path->Upload->DbValue = empty($OldFiles) ? "" : implode(EW_MULTIPLE_UPLOAD_SEPARATOR, $OldFiles);
					$this->path->Upload->FileName = implode(EW_MULTIPLE_UPLOAD_SEPARATOR, $NewFiles);
					...
					if ($this->path->Visible && !$this->path->Upload->KeepFile) {
						...
						if (!ew_Empty($this->path->Upload->FileName)) {
							...
							for ($i = 0; $i < $NewFileCount; $i++) {
								$fldvar = ($this->path->Upload->Index < 0) ? $this->path->FldVar : substr($this->path->FldVar, 0, 1) . $this->path->Upload->Index . substr($this->path->FldVar, 1);
								if ($NewFiles[$i] <> "") {
									$file = ew_UploadTempPath($fldvar, $this->path->TblVar) . $NewFiles[$i];
									if (file_exists($file)) {
