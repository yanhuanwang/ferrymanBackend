...
function CurrentProjectID() {
	...
	return "{D49A959E-F136-47C9-B9D7-6B34755F62D4}";
}
		...
		if (is_array($this->Links)) {
			...
			if (PAGE_TITLE_STYLE == "Caption") {
				...
				for ($i = 0; $i < $cnt; $i++) {
					...
					if ($i < $cnt - 1) {
						$nav .= "<li class=\"breadcrumb-item\" id=\"ewBreadcrumb" . ($i + 1) . "\">";
					} else { // Last => Current page
						$nav .= "<li class=\"breadcrumb-item active\" id=\"ewBreadcrumb" . ($i + 1) . "\">";
...
class HttpUpload
{
	 ...
	 */
	protected function getTempFile($idx = -1)
	{
		if ($this->FileName <> "") {
	...
	public function validateUser(&$usr, &$pwd, $autologin, $encrypted = FALSE, $provider = "")
	{
		...
		if (!$valid) {
			...
			if ($rs = $UserTableConn->execute($sql)) {
				if (!$rs->EOF) {
					$valid = $customValid || ComparePassword($rs->fields('password'), $pwd, $encrypted);
					if ($valid) {
						$_SESSION[SESSION_STATUS] = "login";
...
class OptionValues implements HtmlValueInterface {
	public $Values = [];
	public static $ToHtml = PROJECT_NAMESPACE . "OptionsHtml";
