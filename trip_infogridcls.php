<?php include_once "trip_infoinfo.php" ?>
...
$trip_info_grid = NULL; // Initialize page object first

class ctrip_info_grid extends ctrip_info {
	...
	// Table name
	var $TableName = 'trip_info';

	// Page object name
	var $PageObjName = 'trip_info_grid';

	// Grid form hidden field names
	var $FormName = 'ftrip_infogrid';
	...
	function __construct() {
		...
		if (!isset($GLOBALS["trip_info"]) || get_class($GLOBALS["trip_info"]) == "ctrip_info") {
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["trip_info"];
		...
		if (!defined("EW_TABLE_NAME"))
			define("EW_TABLE_NAME", 'trip_info', TRUE);
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_user_id") && $objForm->HasValue("o_user_id") && $this->user_id->CurrentValue <> $this->user_id->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_flight_number") && $objForm->HasValue("o_flight_number") && $this->flight_number->CurrentValue <> $this->flight_number->OldValue)
			return FALSE;
	...
	function LoadFormValues() {
		...
		if (!$this->flight_number->FldIsDetailKey) {
			$this->flight_number->setFormValue($objForm->GetValue("x_flight_number"));
		}
		$this->flight_number->setOldValue($objForm->GetValue("o_flight_number"));
	...
	function RenderRow() {
		...
		$this->date->ViewCustomAttributes = "";
			...
			$this->flight_number->EditValue = ew_HtmlEncode($this->flight_number->CurrentValue);
			$this->flight_number->PlaceHolder = ew_RemoveHtml($this->flight_number->FldCaption());
	...
	function ValidateForm() {
		...
		if (!$this->flight_number->FldIsDetailKey && !is_null($this->flight_number->FormValue) && $this->flight_number->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->flight_number->FldCaption(), $this->flight_number->ReqErrMsg));
		}
